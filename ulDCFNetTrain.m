...
%% general settings
opts.imdbDir = ['data/imdb/' videoName '_imdb_' num2str(thre(etime)) '.mat'];
...
opts.outDir = fullfile(opts.outDir, [opts.saveModelName ' - r' num2str(thre(etime))]);
...
for i = 1:opts.trainOpts.numEpochs
    ...
    save(fullfile(opts.saveModelDir, [opts.saveModelName ' - e' num2str(i) ' - thre' num2str(thre(etime)) '.mat']), 'net');
end
end