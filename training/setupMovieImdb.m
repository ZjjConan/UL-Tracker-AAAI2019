function imdb = setupMovieImdb(varargin)
    ...
    imdb.images.data = ulDir(opts.imgDir, 'jpg');
    boxFiles = ulDir(opts.boxDir, 'mat');
    nbox = numel(boxFiles);
    ...
    if opts.isSegment
        ...
        % remove empty box
        notok = cellfun(@isempty, imdb.images.bbox, 'UniformOutput', true);
%         if opts.removeBoxWithNumber
%             notok = notok | cellfun(@(x) size(x, 1) < opts.minBoxNumbers, ...
%                 imdb.images.bbox, 'UniformOutput', true);
%         end
        imdb.images.bbox(notok) = [];
        imdb.images.data(notok) = [];
    end
    ...
    ulMakeDir(fileparts(opts.imdbPath));
